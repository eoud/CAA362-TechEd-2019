class zcl_caa362_utility_tpl definition
  public
  final
  create public .
  ...
  public section.
    ...
    class-methods insert_new_flight
      importing value(im_cityfrom) type zcaa362_spfl_tpl-cityfrom
                value(im_cityto)   type zcaa362_spfl_tpl-cityto.

    class-methods print_route
      importing
        i_out       type ref to if_oo_adt_classrun_out
        it_route    type zcl_caa362_graph_tpl=>tt_conn
        iv_cityto   type zcaa362_spfl_tpl-cityto
        iv_cityfrom type zcaa362_spfl_tpl-cityfrom
        iv_type     type string.
...
class zcl_caa362_utility_tpl implementation.
  method insert_new_flight.
    select single from zcaa362_sgeo_tpl fields * where city = @im_cityfrom
    into @data(lv_cityfrom).
    select single from zcaa362_sgeo_tpl fields * where city = @im_cityto
      into @data(lv_cityto).
    select from zcaa362_spfl_tpl fields * into table @data(lt_spfli).

    data(distance) = zcl_caa362_scalar_tpl=>get_linear_distance(
    ...
    insert zcaa362_spfl_tpl from @( value zcaa362_spfl_tpl(
      carrid = 'LH'
      connid = 9000
